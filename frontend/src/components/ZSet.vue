<template>
    <el-card>
        ...
        <el-table :data="value" style="width: 100%">
            ...
            <el-table-column prop="member" label="字段">
            </el-table-column>
...
const showEditDialog = (title: string, index: number) => {
    ...
    if (index >= 0) {
        form.value = props.value[index].score;
        form.field = props.value[index].member;
    }
};

function handleDelete(index: number) {
    let data = [];
    data.push(props.value[index].member);
...
function handleEdit() {
    ...
    if (editDialog.index < 0) {
        addKeyField(connStore.current, dbStore.current, props.name, form).then(() => {
            props.value.push({ "member": form.field, "score": form.value });
            ElMessage({
                type: 'success',
                message: '添加成功',
            })
        ...
        pathcKeyField(connStore.current, dbStore.current, props.name, form).then(() => {
            props.value[editDialog.index].score = form.value
            ElMessage({
                type: 'success',
                message: '修改成功',
            })
